...
bool ltx_text_encoder::encode(const std::string & prompt, int max_tokens, int & n_tokens, std::vector<float> & packed, int & n_hidden, int & n_states) {
    ...
    int n = llama_tokenize(vocab, text.c_str(), (int) text.size(), tokens.data(), (int) tokens.size(), /*add_special*/ true, /*parse_special*/ false);
    if (n < 0) {
        tokens.resize(-n);
        n = llama_tokenize(vocab, text.c_str(), (int) text.size(), tokens.data(), (int) tokens.size(), true, false);
    }
    if (n <= 0) {
...
static ggml_tensor * ltx_connector(ggml_context * ctx, const ltx_model & model, const std::string & prefix, ggml_tensor * x,
                                   ggml_tensor * reg_idx, ggml_tensor * cos_t, ggml_tensor * sin_t, int n_heads, bool flash_attn) {
    ...
    for (int il = 0; il < hp.connector_layers; il++) {
        ...
        ggml_tensor * a = ltx_attention(ctx, aw, h, nullptr, cos_t, sin_t, cos_t, sin_t, n_heads, flash_attn);
        x = ggml_add(ctx, x, a);
...
bool ltx_build_text_cond(const ltx_model & model, mg_backend & be, const std::vector<float> & packed, int n_tokens, int n_hidden, int n_states,
    ...
    std::vector<float> vcos, vsin, acos_, asin_;
    ltx_rope_tables(pos, n_tot, 1, hp.v_dim, hp.rope_theta, vcos, vsin);
    ltx_rope_tables(pos, n_tot, 1, hp.a_dim, hp.rope_theta, acos_, asin_);
    ggml_tensor * t_vcos = g.new_input_f32({ hp.v_dim / 2, n_tot }, vcos, "v_cos");
    ggml_tensor * t_vsin = g.new_input_f32({ hp.v_dim / 2, n_tot }, vsin, "v_sin");
    ggml_tensor * t_acos = g.new_input_f32({ hp.a_dim / 2, n_tot }, acos_, "a_cos");
    ggml_tensor * t_asin = g.new_input_f32({ hp.a_dim / 2, n_tot }, asin_, "a_sin");
    ...
    v = ltx_connector(ctx, model, "video_embeddings_connector", v, t_reg, t_vcos, t_vsin, hp.v_heads, flash_attn);
    a = ltx_connector(ctx, model, "audio_embeddings_connector", a, t_reg, t_acos, t_asin, hp.a_heads, flash_attn);
