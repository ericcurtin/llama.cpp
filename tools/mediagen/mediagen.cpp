...
void mediagen_free(mediagen_context * ctx) {
    ...
    ctx->text.free();
    ctx->ltx.dit.buf.reset();
    ctx->ltx.text_proj.buf.reset();
    ctx->ltx.vae.buf.reset();
    ctx->ltx.audio_vae.buf.reset();
    ctx->be.free();
    delete ctx;
}
    ...
    if (params->enhance_prompt && !prompt.empty()) {
        if (ctx->enhanced_valid && ctx->enhanced_from == prompt) {
            ...
            if (ctx->text.enhance(prompt, 10, 512, enhanced)) {
                const auto t1 = std::chrono::steady_clock::now();
                MG_INF("%s: enhanced prompt in %.1f s: %s\n", __func__, std::chrono::duration<double>(t1 - t0).count(), enhanced.c_str());
