...
bool ltx_text_encoder::enhance(const std::string & prompt, uint32_t seed, int max_new_tokens, std::string & out) {
    ...
    int n_tok = llama_tokenize(vocab, text.c_str(), (int) text.size(), tokens.data(), (int) tokens.size(), true, true);
    if (n_tok < 0) {
        tokens.resize(-n_tok);
        n_tok = llama_tokenize(vocab, text.c_str(), (int) text.size(), tokens.data(), (int) tokens.size(), true, true);
    }
    if (n_tok <= 0) {
        return false;
    }
