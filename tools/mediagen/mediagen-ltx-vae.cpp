...
static ggml_tensor * vae_pixel_norm(ggml_context * ctx, ggml_tensor * x) {
    ...
    p = ggml_rms_norm(ctx, p, 1e-8f);
    return ggml_cont(ctx, ggml_permute(ctx, p, 2, 0, 1, 3));             // [W, H, C, T]
}
...
bool ltx_vae_decode(const ltx_model & model, mg_backend & be, const ltx_latent_video & lat,
    ...
    {
        ggml_tensor * tm = w.get("per_channel_statistics.mean-of-means");
        ggml_tensor * ts = w.get("per_channel_statistics.std-of-means");
        if (tm->ne[0] < C || ts->ne[0] < C) {
            MG_ERR("%s: latent statistics have fewer than %d channels\n", __func__, C);
            return false;
        }
        mean.resize(C);
        std.resize(C);
        std::vector<uint8_t> buf(ggml_nbytes(tm));
        ggml_backend_tensor_get(tm, buf.data(), 0, buf.size());
        for (int c = 0; c < C; c++) {
            mean[c] = tm->type == GGML_TYPE_F32 ? ((float *) buf.data())[c] : ggml_fp16_to_fp32(((ggml_fp16_t *) buf.data())[c]);
        }
        buf.resize(ggml_nbytes(ts));
        ggml_backend_tensor_get(ts, buf.data(), 0, buf.size());
        for (int c = 0; c < C; c++) {
            std[c] = ts->type == GGML_TYPE_F32 ? ((float *) buf.data())[c] : ggml_fp16_to_fp32(((ggml_fp16_t *) buf.data())[c]);
        }
    }
    ...
    for (int f = 0; f < F; f++) {
        for (int h = 0; h < Hl; h++) {
            for (int wi = 0; wi < Wl; wi++) {
                ...
                for (int c = 0; c < C; c++) {
                    x[wi + (size_t) Wl * (h + (size_t) Hl * (c + (size_t) C * f))] = src[c] * std[c] + mean[c];
                }
            }
        }
    }
