    ...
    try {
        for (auto it = j.begin(); it != j.end(); ++it) {
            ...
            if (offs.size() != 2 || offs[1] < offs[0]) {
                MG_ERR("%s: invalid data_offsets for tensor '%s'\n", __func__, d.name.c_str());
                return false;
            }
            d.offset = base + offs[0];
            d.nbytes = offs[1] - offs[0];
...
bool mg_gguf_read_header(const std::string & path, std::vector<mg_tensor_desc> & out, std::map<std::string, std::string> * kv_str) {
    ...
    for (uint64_t i = 0; i < n_tensors && r.ok; i++) {
        ...
        if (d.type < 0 || d.type >= GGML_TYPE_COUNT || ggml_type_size(d.type) == 0) { r.ok = false; break; }
        const int64_t blck = ggml_blck_size(d.type);
        if (d.ne.empty() || d.ne[0] % blck != 0) { r.ok = false; break; }
        d.nbytes = (size_t) (mg_nelements(d.ne) / blck) * ggml_type_size(d.type);
...
bool mg_weights_load(mg_weights & w, const std::string & path, ggml_backend_buffer_type_t buft, const mg_load_opts & opts) {
    ...
    for (auto & p : plan) {
        ...
        const size_t expected = (size_t) (mg_nelements(d.ne) / ggml_blck_size(d.type)) * ggml_type_size(d.type);
        if (d.nbytes != expected) {
            MG_ERR("%s: tensor '%s' has %zu bytes, expected %zu\n", __func__, d.name.c_str(), d.nbytes, expected);
            return false;
        }
        if (p.tap < 0) {
            if (t->type == d.type) {
                ...
                if (opts.transform && t->type == GGML_TYPE_F32) {
                    opts.transform(p.name, (float *) read_buf.data(), ggml_nelements(t));
                }
                ...
                convert_row(d.type, t->type, read_buf.data(), conv_buf.data(), ggml_nelements(t));
                if (opts.transform && t->type == GGML_TYPE_F32) {
                    opts.transform(p.name, (float *) conv_buf.data(), ggml_nelements(t));
                }
                ggml_backend_tensor_set(t, conv_buf.data(), 0, nbytes);
            }
        } else {
            ...
            std::vector<uint8_t> tap_src(blk * IC * OC * src_ts);
            for (int64_t oc = 0; oc < OC; oc++) {
                for (int64_t ic = 0; ic < IC; ic++) {
                    const size_t src_off = (((oc * IC + ic) * KT + p.tap) * blk) * src_ts;
                    const size_t dst_off = ((oc * IC + ic) * blk) * src_ts;
                    memcpy(tap_src.data() + dst_off, read_buf.data() + src_off, blk * src_ts);
                }
            }
            conv_buf.resize(nbytes);
            convert_row(d.type, t->type, tap_src.data(), conv_buf.data(), ggml_nelements(t));
            ggml_backend_tensor_set(t, conv_buf.data(), 0, nbytes);
        }
    }
...
mg_graph::mg_graph(size_t max_nodes) {
    ggml_init_params ip = {
