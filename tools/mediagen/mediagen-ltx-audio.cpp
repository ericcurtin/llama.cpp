...
static ggml_tensor * avae_pixel_norm(ggml_context * ctx, ggml_tensor * x) {
    ggml_tensor * p = ggml_cont(ctx, ggml_permute(ctx, x, 1, 2, 0, 3)); // [C, F, T, 1]
    p = ggml_rms_norm(ctx, p, 1e-6f);
...
bool ltx_audio_decode(const ltx_model & model, mg_backend & be, const ltx_latent_audio & lat,
    ...
    std::vector<float> mean(C * Fq), stdv(C * Fq);
    {
        ggml_tensor * tm = w.get("audio_vae.per_channel_statistics.mean-of-means");
        ggml_tensor * ts = w.get("audio_vae.per_channel_statistics.std-of-means");
        if (tm->type != GGML_TYPE_F32 || ts->type != GGML_TYPE_F32 || tm->ne[0] != C * Fq || ts->ne[0] != C * Fq) {
            MG_ERR("%s: unexpected audio latent statistics\n", __func__);
            return false;
        }
        ggml_backend_tensor_get(tm, mean.data(), 0, ggml_nbytes(tm));
        ggml_backend_tensor_get(ts, stdv.data(), 0, ggml_nbytes(ts));
    }
