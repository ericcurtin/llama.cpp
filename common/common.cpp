...
//
// Docker Hub AI model download utils
//
...
static std::string common_docker_resolve_model(const std::string & docker_url) {
    ...
    try {
        ...
        manifest_params.headers.push_back("Accept: application/vnd.docker.distribution.manifest.v2+json,application/vnd.oci.image.manifest.v1+json");
        ...
        blob_params.headers.push_back("Authorization: Bearer " + token);
        auto blob_res = common_remote_get_content(blob_url, blob_params);
        if (blob_res.first != 200) {
            throw std::runtime_error("Failed to download Docker blob, HTTP code: " + std::to_string(blob_res.first));
        }
